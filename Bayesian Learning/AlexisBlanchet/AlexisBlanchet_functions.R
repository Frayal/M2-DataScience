  ...
  #' 
{
  ...
  for(i in 1:k)
  {
    Respons[,i] <- p[i]*dmnorm(x, mean = Mu[i,], varcov = Sigma[,,i], log=FALSE)## Complete the code
  }
  ...
  #' returns the optimized parametres (Mu, Sigma, p) as a list 
{
  ...
  p <- NK/N ## Complete the code 
  mustar <- (t(respons)%*%x)/NK## Complete the code 
  Sigstar=array(dim=c(d,d,k))
  for( i in (1:k))
  {
    ## Complete the code
    x_tmp = t(t(x) - mustar[i, ])
    Sigstar[,,i] <- t(x_tmp * respons[,i]) %*% x_tmp / NK[i] ## Complete the code
...
{ 
  ...
  {
    niter <- niter+1
    respons <- estep(x , current$Mu ,current$Sigma , current$p) ## responsibilities (n*k matrix)
    ## complete the code
    current <- mstep(x , respons) ## New parameters (complete using M-step)
...
#' Beta: k-vector                   
{
  ...
  M <- (beta0*m0+NK*barx)/Beta ## complete the code: optimal mean parameters m_j for the mu_j's:
  ## a k*d matrix
  d <- ncol(x)
  ...
  for( j in (1:K))
  {
    ...
    Winv[,,j] <- W0inv+as.vector(NK[j])*Sj+(beta0*NK[j]/(beta0+NK[j]))*t(t(barx[j,])-m0)%*%(t(barx[j,])-m0) ## complete the code: optimal W^{-1}
  ...
  #' returns: a n*k matrix: the responsibilities for each data point.  
{
  ...
  logResponsT <-Elogrho+0.5*Eloglambda-0.5*Equadratic ## Complete the code: the transpose of
  ...
  #'   where T is the number of steps.
{
  ...
  while(continue){        
    niter <- niter+1
    respons <- vbEstep(x, current$alpha, current$Winv, current$Nu, current$M, current$Beta)## Complete the coe using vbEstep
    if(nanDetector(respons)) {stop("NaNs detected!\n")}
    vbOpt <-vbMstep(x , respons , alpha0 ,  W0inv, nu0 , m0 , beta0) ## Complete the code using vbMstep
...

{
  ...
  for(j in (1:k))
  {
    ...
    Sigma[,,j] <- rwishart(ppar$nu_Sigma,(1/ppar$nu_Sigma)*Sigma[,,j]) ## complete the code. use function rwishart
  }
    ...
    #' - naccept! number of accepted proposals. 
{
    ...
    for (niter in 1:nsample){
        ...
        lacceptratio <- min(0, (proposal$lpost + ddirichlet(current$p, alphaPropback, log = TRUE) + llkbackSigma) 
                            - (current$lpost + ddirichlet(proposal$p, alphaPropmove, log = TRUE) + llkmoveSigma))  ## logarithm of the acceptance ratio.
  ...
  #' returns: a single numeric value
{
  ...
  inds <- (burnin+1):nsample
  inds <- inds[inds%%thin==0]
  vectllk <- vapply(inds, function(niter){vect_pdf <- vapply(1:k, function(j){
    dmnorm(x,
           mean = sample$Mu[j,,niter],
           varcov = sample$Sigma[,,j,niter])}
    ## complete the code
    , FUN.VALUE = numeric(1)
  )
  sum(sample$p[,niter]*vect_pdf)
  }, FUN.VALUE = numeric(1))
  return(mean(vectllk))
}
  ...
  #' returns: a single numeric value. 
{
  ...
  vectcdf <- vapply(inds, function(niter){gmcdf(x,sample$Mu[,,niter],sample$Sigma[,,,niter],sample$p[,niter])}, FUN.VALUE = numeric(1))
  return(1-mean(vectcdf))
}
